...
Everything a reward needs to be scored is now fixed: the tasks, the seven
generators, the frozen pool, a timing protocol built to survive the measurement
attacks, and the formulations in Table~\ref{tab:rewards}. Every result that
follows is therefore a computation over this one pool - reproducible,
inference-free, and scored on identical samples - so that any difference we
report between rewards is a fact about the rewards, not about decoding. We turn
to those results next.
