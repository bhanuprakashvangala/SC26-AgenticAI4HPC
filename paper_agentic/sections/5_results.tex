...
realizes a mean 8-thread self-speedup of \BoNCorr$\times$---an uninformed pick,
since it ties every candidate at $1$. The reference-anchored gate we advocate
raises this to \BoNGate$\times$ (\BoNGain\% higher). That margin is modest, and
the reason is instructive: the self-anchored variant would instead select the
fastest-scaling program on every task, reaching the oracle at
\BoNGateSelf$\times$ (\BoNGainSelf\% over correctness-only), but it earns that
gain by rewarding self-speedup, which H4 fabricates by slowing the one-thread
baseline (Section~\ref{sec:results-hacks}). The reference anchor gives up most of
the selection gain precisely to close that attack; hack-resistance and
selection sharpness trade against each other, and we take resistance. Either
way the two rewards select a different program in \PoolsDiffering\ of the tasks
with more than one accepted candidate, and correctness-only leaves on the table
speedup that any performance-aware gate recovers at no cost in correctness. The
result of the experiment is not the size of the margin but that the correctness
reward cannot make the choice at all.
...
Archer found \NRaces\ data races in the \NAccepted\ accepted programs, and our
differential sweep correctly caught all \NInjected\ injected race
controls---hazards that pass an execute-once check yet fail validation on nine of
twelve thread/rep runs---confirming that the sweep detects the order-dependence a
single execution misses. Two independent instruments---output-differential and
happens-before---agree that the accepted set is race-free. The failures we
observed from the smallest open-weights models were single-thread semantic
errors rather than data races, so we make no parallel-race claim there.
...
idiomatic tasks. That is precisely why the reward cannot stop there. What that
leaves for anyone building an HPC coding agent today - and what we deliberately
refuse to claim on this evidence - is the subject of the next section.
